...
"""SMTP dispatch mixin for MailProxy.

This module provides the DispatcherMixin class containing all SMTP dispatch
loop logic, email building, and sending functionality.
"""
...
from ..entities.tenant.schema import LargeFileAction, get_tenant_attachment_url
...
class DispatcherMixin:
    """Mixin providing SMTP dispatch loop and email building methods.

    This mixin is designed to be used with MailProxy and assumes access to:
    - self.db: Database adapter
    - self.pool: SMTP connection pool
    - self.rate_limiter: Rate limiting service
    - self.metrics: Prometheus metrics
    - self.attachments: Global AttachmentManager
    - self._retry_strategy: Retry configuration
    - self.logger: Logger instance
    - Various configuration attributes (_smtp_batch_size, etc.)
    """
    ...
    if TYPE_CHECKING:
        ...
        _wake_cleanup_event: asyncio.Event
        _test_mode: bool
        _send_loop_interval: int
        _smtp_batch_size: int
        _batch_size_per_account: int
        _max_concurrent_sends: int
        _max_concurrent_per_account: int
        _max_concurrent_attachments: int
        _attachment_timeout: float
        _attachment_semaphore: asyncio.Semaphore | None
        _attachment_cache: Any
        ...
        default_use_tls: bool | None

    async def _smtp_dispatch_loop(self: MailProxy) -> None:
    ...
    def _get_account_semaphore(self: MailProxy, account_id: str) -> asyncio.Semaphore:
    ...
    async def _process_smtp_cycle(self: MailProxy) -> bool:
    ...
    async def _dispatch_batch(self: MailProxy, batch: list[dict[str, Any]], now_ts: int) -> None:
    ...
    async def _dispatch_message(self: MailProxy, entry: dict[str, Any], now_ts: int) -> None:
    ...
    async def _build_email(self: MailProxy, data: dict[str, Any]) -> tuple[EmailMessage, str]:
        ...
        """
        from .proxy import AttachmentTooLargeError
    ...
    async def _get_large_file_config_for_message(self: MailProxy, data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def _create_large_file_storage(self: MailProxy, config: dict[str, Any]) -> LargeFileStorage | None:
    ...
    async def _wait_for_wakeup(self: MailProxy, timeout: float | None) -> None:
    ...
    async def _cleanup_loop(self: MailProxy) -> None:
