...
class TestFetchBatch:
    """Tests for batch fetch operation."""

    @pytest.mark.asyncio
    async def test_batch_single_item_uses_simple_fetch(self):
        """Test that single item doesn't trigger batch logic."""
        fetcher = HttpFetcher(default_endpoint="https://api.example.com")
        expected_content = b"single file"
    ...
    @pytest.mark.asyncio
    async def test_batch_multiple_items_json_response(self):
        """Test batch fetch with JSON response format."""
        fetcher = HttpFetcher(default_endpoint="https://api.example.com")
    ...
    async def test_batch_groups_by_server(self):
        """Test that batch groups items by server URL."""
        fetcher = HttpFetcher(default_endpoint="https://default.example.com")

        content_default = b"from default"
        content_other = b"from other"

        def create_post_context_manager(url, **kwargs):
            """Create a proper async context manager for post calls."""
            mock_response = AsyncMock()
            mock_response.raise_for_status = MagicMock()
            if "other.com" in url:
                mock_response.read = AsyncMock(return_value=content_other)
            else:
                mock_response.read = AsyncMock(return_value=content_default)
        ...
        mock_session = MagicMock()
        mock_session.post = create_post_context_manager
