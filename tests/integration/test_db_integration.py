...
class TestForeignKeyConstraints:
    ...
    async def test_cascade_behavior(self, pg_db):
        """Verify accounts are deleted when tenant is deleted (if cascade enabled)."""
        tenants = pg_db.table("tenants")
        accounts = pg_db.table("accounts")
        ...
        # Account should also be gone (CASCADE) or FK should have blocked delete
        # This depends on schema definition - verify current behavior
        acc_list = await accounts.list_all(tenant_id="cascade-test")
        # Either cascade deleted it or FK prevented tenant deletion
        # Both behaviors are valid depending on schema constraints
        assert len(acc_list) == 0 or await tenants.get("cascade-test") is not None
