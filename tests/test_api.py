...
def test_returns_500_when_service_missing():
    create_app(DummyService())
    api.service = None
    client = TestClient(api.app)
    response = client.post("/command", json={"cmd": "noop"})
    assert response.status_code == 500
    assert response.json()["detail"] == "Service not initialized"


def test_basic_endpoints_dispatch_to_service():
    svc = DummyService()
    client = TestClient(create_app(svc))

    assert client.get("/status").json() == {"ok": True}

    legacy_payload = {"cmd": "run now", "payload": {"value": 1}}
    assert client.post("/command", json=legacy_payload).json() == {"ok": True, "cmd": "run now", "payload": {"value": 1}}
