...
async def test_mark_bounced_creates_event(tmp_path):
    ...
    await db.table('message_events').add_event(
        pk,
        "bounce",
        bounce_ts,
    ...
    events = await db.table('message_events').get_events_for_message(pk)
...
async def test_bounce_event_in_unreported_events(tmp_path):
    ...
    await db.table('message_events').mark_reported(sent_event_ids, sent_ts + 10)
    ...
    unreported = await db.table('message_events').fetch_unreported(limit=10)
...
async def test_mark_bounce_reported(tmp_path):
    ...
    await db.table('message_events').add_event(
        ...
        metadata={"bounce_type": "soft", "bounce_code": "421"},
    )
    ...
    await db.table('message_events').mark_reported([bounce_event_id], reported_ts)
...
async def test_fetch_unreported_includes_both_new_and_bounce(tmp_path):
    ...
    await db.table('message_events').add_event(pk_map["msg-new"], "sent", sent_ts)

    # msg-bounced: sent, reported, then bounced
    await db.table('message_events').add_event(pk_map["msg-bounced"], "sent", sent_ts)

    # Report only the sent event for msg-bounced
    events = await db.table('message_events').fetch_unreported(limit=10)
    bounced_sent_event = [e for e in events if e["message_id"] == "msg-bounced"][0]
    await db.table('message_events').mark_reported([bounced_sent_event["event_id"]], sent_ts + 10)
