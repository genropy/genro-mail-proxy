...
async def test_add_messages_and_dispatch(tmp_path):
    ...
    messages = await core.db.table('messages').list_all(tenant_id="test-tenant")
    ...
    pk = messages[0]["pk"]
    events = await core.db.table('message_events').get_events_for_message(pk)
    event_ids = [e["event_id"] for e in events]
    await core.db.table('message_events').mark_reported(event_ids, past_ts)

    await core._apply_retention()
    assert await core.db.table('messages').list_all(tenant_id="test-tenant") == []
...
async def test_rate_limited_message_is_deferred(tmp_path):
    ...
    ready = await core.db.table('messages').fetch_ready(limit=5, now_ts=core._utc_now_epoch())
    assert ready == []
...
async def test_temporary_error_retry_exhaustion(tmp_path):
    ...
    for attempt in range(4):
        ...
        if attempt > 0:
            ...
            msg = await core.db.table('messages').get("msg-retry-exhausted", "test-tenant")
            if msg:
                await core.db.table('messages').clear_deferred(msg["pk"])
...
async def test_cleanup_messages_command(tmp_path):
    ...
    events = await core.db.table('message_events').get_events_for_message(pk)
    event_ids = [e["event_id"] for e in events]
    await core.db.table('message_events').mark_reported(event_ids, old_ts)
