...
async def test_full_roundtrip_single_tenant(tmp_path, smtp_server, smtp_handler):
    ...
    messages = await core.db.table('messages').list_all(tenant_id="acme")
...
async def test_roundtrip_with_smtp_error(tmp_path, smtp_server, smtp_handler):
    ...
    messages = await core.db.table('messages').list_all(tenant_id="error-tenant")
    assert len(messages) == 1
    assert messages[0]["smtp_ts"] is not None
...
async def test_roundtrip_tenant_sync_failure_retry(tmp_path, smtp_server, smtp_handler):
    ...
    messages = await core.db.table('messages').list_all(tenant_id="flaky-tenant")
    pk = messages[0]["pk"]
    events = await core.db.table('message_events').get_events_for_message(pk)
    assert len(events) == 1
