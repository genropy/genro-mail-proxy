...
async def test_process_client_cycle_routes_to_tenants(tmp_path):
    ...
    inserted = await core.db.table('messages').insert_batch([
        {
            "id": "msg1",
            "tenant_id": "tenant1",
            "account_id": "acc1",
    ...
    await core.db.table('message_events').add_event(pk_map["msg1"], "sent", sent_ts)
    await core.db.table('message_events').add_event(pk_map["msg2"], "sent", sent_ts)
    ...
    with aioresponses() as m:
        ...
        unreported = await core.db.table('message_events').fetch_unreported(limit=10)
...
async def test_process_client_cycle_fallback_global(tmp_path):
    ...
    sent_ts = core._utc_now_epoch()
    await core.db.table('message_events').add_event(inserted[0]["pk"], "sent", sent_ts)
...
async def test_process_client_cycle_mixed_tenants(tmp_path):
    ...
    sent_ts = core._utc_now_epoch()
    await core.db.table('message_events').add_event(pk_map["msg-with-tenant"], "sent", sent_ts)
    await core.db.table('message_events').add_event(pk_map["msg-no-url"], "sent", sent_ts)
...
async def test_process_client_cycle_partial_failure(tmp_path):
    ...
    sent_ts = core._utc_now_epoch()
    await core.db.table('message_events').add_event(pk_map["msg-success"], "sent", sent_ts)
    await core.db.table('message_events').add_event(pk_map["msg-fail"], "sent", sent_ts)
    ...
    with aioresponses() as m:
        ...
        success_events = await core.db.table('message_events').get_events_for_message(pk_map["msg-success"])
        assert len(success_events) == 1
        assert success_events[0]["reported_ts"] is not None

        # Failed tenant event should NOT be marked (will retry next cycle)
        fail_events = await core.db.table('message_events').get_events_for_message(pk_map["msg-fail"])
