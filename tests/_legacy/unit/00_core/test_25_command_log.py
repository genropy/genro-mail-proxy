...
async def test_command_log_basic_crud(tmp_path):
    ...
    # List commands
    commands = await p.table('command_log').list_commands()
...
async def test_command_log_filters(tmp_path):
    ...
    # Filter by tenant
    acme_cmds = await p.table('command_log').list_commands(tenant_id="acme")
    assert len(acme_cmds) == 1
    assert acme_cmds[0]["endpoint"] == "POST /commands/add-messages"

    # Filter by endpoint
    tenant_cmds = await p.table('command_log').list_commands(endpoint_filter="tenant")
    assert len(tenant_cmds) == 1
    assert tenant_cmds[0]["endpoint"] == "POST /tenant"

    # Filter by time range
    recent_cmds = await p.table('command_log').list_commands(since_ts=now - 60)
    assert len(recent_cmds) == 2  # POST /tenant and DELETE /account

    old_cmds = await p.table('command_log').list_commands(until_ts=now - 60)
...
async def test_command_log_export(tmp_path):
    ...
    exported = await p.table('command_log').export_commands()
...
async def test_command_log_purge(tmp_path):
    ...
    # Purge old commands
    deleted = await p.table('command_log').purge_before(now - 600)
    assert deleted == 1

    # Verify remaining
    remaining = await p.table('command_log').list_commands()
