...
async def test_messages_lifecycle(tmp_path):
    ...
    await p.table('tenants').add({"id": "test_tenant", "name": "Test"})
    now = int(time.time())
    inserted = await p.table('messages').insert_batch(
        [
            {
                "id": "msg1",
                "tenant_id": "test_tenant",
    ...
    ready = await p.table('messages').fetch_ready(limit=10, now_ts=now)
    assert len(ready) == 1
    assert ready[0]["id"] == "msg1"
    assert ready[0]["pk"] == pk

    # Test deferral - use pk for internal operations
    await p.table('messages').set_deferred(pk, now + 60)
    assert await p.table('messages').fetch_ready(limit=10, now_ts=now) == []
    await p.table('messages').clear_deferred(pk)
    ready = await p.table('messages').fetch_ready(limit=10, now_ts=now)
    assert len(ready) == 1

    # Test error - marks message as processed with smtp_ts
    await p.table('message_events').add_event(pk, "error", now, description="boom")
    # Message is no longer ready (smtp_ts is set)
    assert await p.table('messages').fetch_ready(limit=10, now_ts=now + 120) == []

    # Verify error event was created
    events = await p.table('message_events').fetch_unreported(limit=10)
    error_events = [e for e in events if e["event_type"] == "error"]
    assert len(error_events) == 1
    assert error_events[0]["description"] == "boom"

    # Test sent - updates smtp_ts
    await p.table('message_events').add_event(pk, "sent", now + 1)
    ...
    await p.table('message_events').mark_reported(event_ids, now + 2)

    # Retention cleanup via events
    removed = await p.table('messages').remove_fully_reported_before(now + 10)
    assert removed == 1
    assert await p.table('messages').list_all(tenant_id="test_tenant") == []
...
async def test_existing_ids(tmp_path):
    ...
    await p.init_db()
    await p.table('tenants').add({"id": "test_tenant", "name": "Test"})
    await p.table('messages').insert_batch(
        [
            {
                "id": "msg1",
                "tenant_id": "test_tenant",
    ...
    existing = await p.table('messages').existing_ids(["msg1", "msg2"])
...
async def test_send_log_and_counts(tmp_path):
    ...
    p = MailProxyDb(str(db))
    await p.init_db()
    await p.table('send_log').log("acc", 10)
    await p.table('send_log').log("acc", 20)
    await p.table('send_log').log("other", 25)
    assert await p.table('send_log').count_since("acc", 15) == 1
    assert await p.table('send_log').count_since("acc", 5) == 2
    assert await p.table('send_log').count_since("acc", 25) == 0
...
async def test_fetch_ready_messages_priority_filter(tmp_path):
    ...
    immediate = await p.table('messages').fetch_ready(limit=10, now_ts=now, priority=0)
    assert len(immediate) == 2
    assert all(m["priority"] == 0 for m in immediate)

    # Fetch only min_priority >= 1 (non-immediate)
    regular = await p.table('messages').fetch_ready(limit=10, now_ts=now, min_priority=1)
    ...
    # Fetch all (no filter)
    all_msgs = await p.table('messages').fetch_ready(limit=10, now_ts=now)
