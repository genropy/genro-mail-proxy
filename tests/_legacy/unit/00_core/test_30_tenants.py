...
async def test_delete_tenant_cascades(tmp_path):
    ...
    assert len(await db.table('messages').list_all(tenant_id="acme")) == 1
    ...
    assert len(await db.table('messages').list_all(tenant_id="acme")) == 0
...
async def test_events_for_tenant_account(tmp_path):
    ...
    await db.table('message_events').add_event(pk, "sent", sent_ts)

    # Fetch unreported events
    events = await db.table('message_events').fetch_unreported(limit=10)
    assert len(events) == 1
    assert events[0]["message_id"] == "msg1"

    # Get message and verify tenant association via account
    msg = await db.table('messages').get("msg1", "acme")
...
async def test_events_multiple_tenants(tmp_path):
    ...
    # Mark all as sent via events (triggers update message state)
    sent_ts = int(time.time())
    for msg_id in ["msg1", "msg2", "msg3"]:
        await db.table('message_events').add_event(pk_by_id[msg_id], "sent", sent_ts)
    ...
    for e in events:
        ...
        msg = await db.table('messages').get(e["message_id"], "tenant1")
        if msg:
            tenant1_msgs.append(e["message_id"])
        else:
            msg = await db.table('messages').get(e["message_id"], "tenant2")
            if msg:
...
async def test_count_pending_for_tenant(tmp_path):
    ...
    count = await db.table('messages').count_pending_for_tenant("acme")
    assert count == 4

    # Count specific batch
    count_nl01 = await db.table('messages').count_pending_for_tenant("acme", "NL-01")
    assert count_nl01 == 2

    count_nl02 = await db.table('messages').count_pending_for_tenant("acme", "NL-02")
    assert count_nl02 == 1

    # Mark one as sent via event
    await db.table('message_events').add_event(pk_by_id["msg-1"], "sent", now_ts)
