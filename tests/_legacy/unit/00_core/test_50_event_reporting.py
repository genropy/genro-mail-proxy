...
class TestEventReportingCycle:
    ...
    async def test_cycle_processes_events_and_marks_reported(self, proxy: MailProxy):
        ...
        await proxy.db.table('message_events').add_event(message_pk=pk, event_type="sent", event_ts=1700000000)
        ...
        events = await proxy.db.table('message_events').fetch_unreported(limit=10)
        assert len(events) == 0
...
class TestProcessClientCycleWithTenants:
    ...
    async def test_cycle_groups_events_by_tenant(self, proxy: MailProxy):
        ...
        await proxy.db.table('message_events').add_event(message_pk=pk_map["msg-a1"], event_type="sent", event_ts=1700000000)
        await proxy.db.table('message_events').add_event(message_pk=pk_map["msg-b1"], event_type="sent", event_ts=1700000100)
